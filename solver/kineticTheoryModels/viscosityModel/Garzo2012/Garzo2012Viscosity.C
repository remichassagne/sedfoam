...
) const
{
    ...
    const dimensionedScalar mu4_0 = (9./2+pow(e, 2));
    ...
    const volScalarField muk = alpha*pow(Theta, 0.5) *
            ...
            (
             96/(5*sqrtPi)*((1-0.25*pow((1-e), 2))*(1+7./16*a2) 
                 - 5./24*(1-pow(e, 2))*(1+3*a2/16))*g0*alpha*pow(Theta, 0.5) 
             + (1-alpha)*da*K/rhoa
            );
    //Contact viscosity
...
) const
{
    ...
    (
        4./(3*sqrtPi)*(1+e)*(1-a2/16)*pow(alpha, 2)*g0
    );
}
